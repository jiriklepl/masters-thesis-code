...
instance Pretty (Section a) where
    ...
    pretty (SecSpan left right sections) = pretty left <+> pretty right <+> bracesBlock sections

instance Pretty (Decl a) where
    pretty (ImportDecl imports) = "import" <+> commaSep (pretty <$> imports) <> semi
    pretty (ExportDecl exports) = "export" <+> commaSep (pretty <$> exports) <> semi
    pretty (ConstDecl mType name expr) = "const" <+> maybeSpacedR mType <> pretty name <+> equals <+> pretty expr <> semi
    pretty (TypedefDecl type_ names) = "typedef" <+> pretty type_ <+> commaSep (pretty <$> names) <> semi
    pretty (RegDecl invar registers) = ifTrue invar ("invariant" <> space) <> pretty registers <> semi
    pretty (PragmaDecl name pragma) = "pragma" <+> pretty name <+> braces (pretty pragma)
    pretty (TargetDecl targetDirectives) = "target" <+> hsep (pretty <$> targetDirectives) <> semi

instance Pretty (TargetDirective a) where
    pretty (MemSize int) = "memsize" <+> pretty int
    pretty (ByteOrder endian) = "byteorder" <+> (\case Little -> "little"; Big -> "big") endian
...
instance Pretty (Datum a) where
    ...
    pretty (Datum type_ mSize mInit) = pretty type_ <> maybe mempty pretty mSize <> maybe mempty pretty mInit <> semi
...
instance Pretty (Procedure a) where
    pretty (Procedure mConv name formals body) = maybeSpacedR mConv <> pretty name <> parens (commaSep $ pretty <$> formals) <+> pretty body

instance Pretty (Formal a) where
    pretty (Formal mKind invar type_ name) = maybeSpacedR mKind <> ifTrue invar ("invariant" <> space) <> pretty type_ <+> pretty name
...
instance Pretty (Stmt a) where
    ...
    pretty (IfStmt cond ifBody mElseBody) = "if" <+> pretty cond <+> pretty ifBody <> maybe mempty ((space <>) . ("else" <+>) . pretty) mElseBody
    pretty (SwitchStmt expr arms) = "switch" <+> pretty expr <+> bracesBlock arms
    pretty (SpanStmt left right body) = "span" <+> pretty left <+> pretty right <+> pretty body
    pretty (AssignStmt lvalues exprs) = commaSep (pretty <$> lvalues) <+> equals <+> commaSep (pretty <$> exprs) <> semi
    pretty (PrimOpStmt left opName actuals flows) = pretty left <+> equals <+> "%%" <> pretty opName <> parens (commaSep $ pretty <$> actuals) <> hsep (pretty <$> flows) <> semi
    ...
    pretty (CutToStmt expr actuals flows) = "cut" <+> "to" <+> pretty expr <> parens (commaSep $ pretty <$> actuals) <> hsep (pretty <$> flows) <> semi

instance Pretty (KindName a) where
    pretty (KindName mKind name) = maybeSpacedR mKind <> pretty name

instance Pretty (Arm a) where
    pretty (Arm ranges body) = "case" <+> commaSep (pretty <$> ranges) <> colon <+> pretty body
...
instance Pretty (LValue a) where
    ...
    pretty (LVRef type_ expr mAsserts) = pretty type_ <> brackets (pretty expr <> maybe mempty (\asserts -> space <> pretty asserts) mAsserts)

instance Pretty (Flow a) where
    pretty (AlsoCutsTo names) = "also" <+> "cuts" <+> "to" <+> commaSep (pretty <$> names)
    pretty (AlsoUnwindsTo names) = "also" <+> "unwinds" <+> "to" <+> commaSep (pretty <$> names)
    pretty (AlsoReturnsTo names) = "also" <+> "returns" <+> "to" <+> commaSep (pretty <$> names)
...
instance Pretty (Expr a) where
    pretty (LitExpr lit mType) = pretty lit <> maybe mempty ((space <>) . ("::" <+>) . pretty) mType
    pretty (LVExpr lvalue) = pretty lvalue
    pretty (ParExpr expr) = parens $ pretty expr
    pretty (BinOpExpr op left right) = pretty left <+> prettyOp op <+> pretty right
        where
            prettyOp AddOp = "+"
            prettyOp SubOp = "-"
            prettyOp MulOp = "*"
    ...
    pretty (InfixExpr name left right) = pretty left <+> "`" <> pretty name <> "`" <+> pretty right
    pretty (PrefixExpr name actuals) = "%" <> pretty name <> parens (commaSep $ pretty <$> actuals)
...
instance Pretty (Asserts a) where
    ...
    pretty (AlignAssert int names) = "aligned" <+> pretty int <+> "in" <+> commaSep (pretty <$> names)
    pretty (InAssert names mInt) = "in" <+> commaSep (pretty <$> names) <> maybeSpacedL mInt
