...
import safe Control.Monad.State.Lazy ( evalState, MonadState(put, get) )
...
type FlattenAnnot4 n1 n2 n3 n4 = (FlattenAnnot3 n1 n2 n3, FlattenAnnot n4)
type FlattenAnnot5 n1 n2 n3 n4 n5 = (FlattenAnnot4 n1 n2 n3 n4, FlattenAnnot n5)
type FlattenAnnot6 n1 n2 n3 n4 n5 n6 = (FlattenAnnot5 n1 n2 n3 n4 n5, FlattenAnnot n6)
...
instance FlattenAnnot3 Section Procedure Expr => Flatten Section where
    ...
    flatten (SecSpan left right items) = SecSpan (flatten left) (flatten right) (flatten <$> items)
...
instance FlattenBodyItems (Annot BodyItem) => FlattenBodyItems (Annot Body) where
    flattenBodyItems [] = pure []
    flattenBodyItems (Annot (Body bodyItems) _ : bodies) = liftA2 (++) (flattenBodyItems bodyItems) (flattenBodyItems bodies)

class FlattenStmt n where
    flattenStmt :: MonadState Int m => n a -> m [Annot BodyItem a]

instance (FlattenStmt (Annot BodyItem)) =>  FlattenBodyItems (Annot BodyItem) where
    flattenBodyItems [] = pure []
    flattenBodyItems (decl@(Annot BodyDecl{} _) : bodyItems) = (decl :) <$> flattenBodyItems bodyItems
    flattenBodyItems (stackDecl@(Annot BodyStackDecl{} _) : bodyItems) = (stackDecl :) <$> flattenBodyItems bodyItems
    flattenBodyItems (stmt : bodyItems) = liftA2 (++) (flattenStmt stmt) (flattenBodyItems bodyItems)

instance {-# OVERLAPPING #-} FlattenStmt (Annot Stmt) => FlattenStmt (Annot BodyItem) where
...
trivialGoto a = withAnnot a . flip GotoStmt Nothing . withAnnot a . LVExpr . withAnnot a . LVName

brCond :: Annot Expr a -> Name a -> Name a -> a -> Annot Stmt a
brCond cond tName eName a = withAnnot a $ IfStmt cond
...
instance (FlattenBodyItems (Annot Body)) => FlattenStmt (Annot Stmt) where
    flattenStmt (Annot (IfStmt cond tBody Nothing) a) = do
        ...
        pure $ toBodyStmt (brCond cond tName fName a) : (toBodyStmt . withAnnot a $ LabelStmt tName) : tTransl ++ [toBodyStmt . withAnnot a $ LabelStmt fName]
    flattenStmt (Annot (IfStmt cond tBody (Just eBody)) a) = do
        ...
        pure $ toBodyStmt (brCond cond tName eName a) : (toBodyStmt . withAnnot a $ LabelStmt tName) : tTransl ++ toBodyStmt (trivialGoto a fName) : (toBodyStmt . withAnnot a $ LabelStmt eName) : eTransl ++ [toBodyStmt . withAnnot a $ LabelStmt fName]
    flattenStmt (Annot (SwitchStmt expr arms) annot) = do
        num <- show <$> fresh
        let endName = Name . T.pack $ "switch_" ++ num ++ "_end"
        let caseNames = Name . T.pack . (("switch_" ++ num ++ "_") ++) . show <$> take (length arms) [(1 :: Int)..]
        armsTransl <- sequence [((toBodyStmt . withAnnot a $ LabelStmt caseName):) . reverse . ((toBodyStmt $ trivialGoto annot endName):) . reverse <$> flattenBodyItems [body] | (Annot (Arm _ body) a, caseName) <- zip arms caseNames]
        let newArms = [withAnnot a . Arm ranges . toBody . toBodyStmt $ trivialGoto a caseName | (Annot (Arm ranges _) a, caseName) <- zip arms caseNames]
        pure $ toBodyStmt (withAnnot annot $ SwitchStmt expr newArms) : concat armsTransl ++ [toBodyStmt . withAnnot annot $ LabelStmt endName]
    flattenStmt (Annot (SpanStmt lExpr rExpr body) annot) = do
        bodyTransl <- flattenBodyItems [body]
        pure [toBodyStmt . withAnnot annot . SpanStmt lExpr rExpr . withAnnot annot $ Body bodyTransl]
