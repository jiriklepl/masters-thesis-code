...
data Context
  ...
  | ClassCtx
      { ctxName :: Text
      , ctxElab :: Elaboration
      , ctxConstraint :: (Text, Type)
      , superSupers :: [(Text, Type)]
      } -- ^ class name, class type properties, super-class type properties
  | FunctionCtx
      { ctxName :: Text
      , ctxElab :: Elaboration
      , ctxFunctionHandle :: Properties
      , ctxFunctionConv :: Maybe Conv
      } -- ^ function name, function type properties, the call convention (eg foreign "C")
  | InstanceCtx
      { ctxName :: Text
      , ctxElab :: Elaboration
      , ctxConstraint :: (Text, Type)
      , ctxSupers :: [(Text, Type)]
      } -- ^ class name, class type properties, assumptions type properties
  | StructCtx
      { ctxName :: Text
      , ctxElab :: Elaboration
      , ctxConstraint :: (Text, Type)
