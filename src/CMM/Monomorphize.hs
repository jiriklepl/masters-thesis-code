...
import safe Control.Lens (_1, _2, uses, zoom)
...
import safe CMM.Inference.Preprocess.Elaboration
  ( Elaboration(LVInstElaboration, MemberElaboration,
...
import safe CMM.Inference.Type as Type (Type(ComplType), makeAddrType)
import safe CMM.Inference.TypeCompl (TypeCompl(AddrType))
...
import safe CMM.Monomorphize.Error
  ( MonomorphizeError(CannotInstantiate, IllegalScheme,
                  ReachedMaxWaves)
  , absurdType
  , illegalHole
  , illegalNothing
...
monomorphizeMaybes subst annot constr pars = do
  sequence <$> traverse (monomorphize subst) pars >>@= \pars' ->
...
cycleWaves node = do
  ...
  more <-
    useMonomorphizer . uses State.polyGenerate $ concatMap submergeTuple .
    Map.toList .
    State.getPolyGenerate
      ...
      | otherwise -> do
        useMonomorphizer State.getMaxWaves >>= \maxWaves ->
          if waves < maxWaves
            ...
            else returnError (head topLevels') $ ReachedMaxWaves waves
      where topLevels' = schematized2topLevel <$> catMaybes mores
            unit = addTopLevels topLevels'

instance (GetPos a, HasElaboration a) =>
         Monomorphize (Annot AST.TopLevel) a where
  monomorphize subst (topLevel `Annot` a) =
    case topLevel of
      ...
      AST.TopSection name items ->
        sequence <$> traverse (monomorphize subst) items >>@= return . Just .
        withAnnot a .
        AST.TopSection name .
        catMaybes
...
instance (GetPos a, HasElaboration a) =>
         Monomorphize (Annot AST.Section) a where
  ...
  monomorphize subst (decl `Annot` a) =
    case decl of
      AST.ImportDecl imports ->
        monomorphizeTrivials subst a AST.ImportDecl imports
      AST.ExportDecl exports ->
        monomorphizeTrivials subst a AST.ExportDecl exports
...
instance (GetPos a, HasElaboration a) =>
         Monomorphize (Annot AST.Registers) a where
  monomorphize subst (AST.Registers mKind t nameStrs `Annot` a) = do
    ...
    return $ do
      ...
      return $ withAnnot b <$>
        liftA2 (AST.Registers mKind) t'' ((`zip` strs) <$> sequence names'')
...
instance (GetPos a, HasElaboration a) =>
         Monomorphize (Annot AST.Instance) a where
  monomorphize subst =
    \case
  ...
  monomorphize subst annotated@(struct `Annot` a) =
    case struct of
      AST.Struct name datums -> do
        props <-
          useInferencer . fmap toTypeVar . State.getProps $ getElaborationId a
        schemeName <- useInferencer . State.fromOldName $ getElaborationId a
        addScheme <-
          useInferencer (lookupScheme schemeName) >>= \case
        ...
        addScheme `reThrow` \_ ->
          useInferencer (typePolytypeness subst $ getElaboration a) >>= \case
            Mono -> do
...
instance HasElaboration a => Monomorphize (Annot AST.Init) a where
  monomorphize subst (init' `Annot` a) = do
    ...
    case init' of
      ...
      AST.Str16Init strLit ->
        succeed . Just . withAnnot b $ AST.Str16Init strLit
...
unifFacts (fact:others) subst =
  ...
  where
    go t t' =
      unifFacts others . (`apply` subst) . fst $
      fromRight logicError (apply subst t `unify` apply subst t')

instance (GetPos a, HasElaboration a) =>
         Monomorphize (Annot AST.Procedure) a where
    ...
    subst' <-
      useInferencer (lookupScheme schemeName) >>= \case
        ...
        Just scheme@(_ :. facts :=> t) -> do
          let subst' =
                either logicError ((`apply` subst) . fst) (t `unify` inst)
          useMonomorphizer $
            State.addPolyScheme
    ...
    addMethod <-
      case getElaboration a of
        ...
        MethodElaboration inst' scheme _ ->
          useMonomorphizer . fmap Right $
          State.addMethod (toTypeVar scheme) (toTypeVar inst')
            ...
            True -> do
              addMemory <-
                ...
                case getElaboration a of
                  ...
                  MethodElaboration _ scheme _ ->
                    Right <$> State.memorize (toTypeVar scheme) inst
                  hole' -> failure $ hole' `illegalHole` annotated
...
instance Monomorphize (Annot AST.ProcedureHeader) a where
  monomorphize subst (AST.ProcedureHeader mConv name formals semis `Annot` a) = do
    ...
    return $ do
      ...
      return $ withAnnot b <$>
        liftA2
          (AST.ProcedureHeader mConv name)
          (sequence formals'')
          (traverse sequence semis'')
...
instance (GetPos a, HasElaboration a) =>
         Monomorphize (Annot AST.BodyItem) a where
...
instance Monomorphize (Annot AST.StackDecl) a where
  monomorphize subst =
    \case
      AST.StackDecl items `Annot` a ->
        monomorphizeTrivials subst a AST.StackDecl items
    ...
    case stmt of
      ...
      AST.IfStmt cond tBody eBody -> do
        ...
        return $ do
          ...
          return $ annotate <$>
            liftA3 AST.IfStmt cond'' tBody'' (sequence eBody'')
      ...
      AST.CallStmt rets mConv func actuals mTargs callAnnots -> do
        ...
        callAnnots' <- traverse (ensuredJustMonomorphize subst) callAnnots
        return $ do
          rets'' <- sequence rets'
          func'' <- func'
      ...
      AST.ReturnStmt mConv Nothing actuals -> do
        ...
        return $ do
          actuals'' <- sequence actuals'
          return $ annotate . AST.ReturnStmt mConv Nothing <$>
            sequence actuals''
      ...
      _ ->
        case lValue of
            ...
            case getElaboration a of
              ...
              LVInstElaboration props scheme ->
                useInferencer (typePolytypeness subst scheme) >>= \case
                  ...
                  _ -> do
                    inst <-
                      useInferencer $ State.reconstructOld (toTypeVar props) >>=
                      simplify .
                      apply subst >>=
                      fmap toTypeVar .
                      State.getProps
          ...
          AST.LVRef mType expr mAsserts -> do
            mType' <- traverse (ensuredJustMonomorphize subst) mType
            expr' <- ensuredJustMonomorphize subst expr
            mAsserts' <- traverse (ensuredJustMonomorphize subst) mAsserts
            return $ do
          ...
          MethodElaboration _ scheme inst ->
            useInferencer (typePolytypeness subst scheme) >>= \case
              ...
              _ -> do
                ...
                inst' <-
                  useInferencer $ State.reconstructOld (toTypeVar inst) >>=
                  simplify .
                  apply subst >>=
                  fmap toTypeVar .
                  State.getProps
  ...
  where
    go =
      \case
        ...
        [] -> fallback
    fallback =
      recallMethod scheme (fst inst) >>= \case
        False -> monomorphizeMethodInner scheme inst
        True -> return $ Right Nothing

-- recallMethod
recallMethod :: TypeVar -> TypeVar -> InferMonomorphizer a Bool
recallMethod scheme item = do
  inst' <-
    useInferencer $ State.reconstructOld (toTypeVar item) <&> makeAddrType
...
monomorphizeMethod scheme inst = do
  useMonomorphizer
    (uses State.polyMethods $ Map.lookup scheme . State.getPolyMethods) >>= \case
...
recallField :: TypeVar -> TypeVar -> InferMonomorphizer a Bool
recallField scheme item = do
  inst' <- useInferencer $ State.reconstructOld (toTypeVar item)
  useMonomorphizer $ State.isMemorized scheme inst'
...
instantiationError scheme inst instWrapper err =
  failure . Error.makeError . CannotInstantiate scheme inst err $
  voidWrapped instWrapper
...
illegalScheme ::
     (Monad m, GetPos a1)
  => Schematized a2
  -> Annotation ASTWrapper a1
  -> m (Either Error b)
illegalScheme schematized instWrapper =
  failure . makeError (takeAnnot instWrapper) . IllegalScheme (void schematized) $
  voidWrapped instWrapper
