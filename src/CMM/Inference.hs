...
class Unify a where
  unify :: MonadWriter ([InferenceError] -> [InferenceError]) m => a -> a -> m (Subst, a)
  unify a b = unifyImpl a b <*> return a
  unifyImpl :: MonadWriter ([InferenceError] -> [InferenceError]) m => a -> a -> m (a -> (Subst, a))
